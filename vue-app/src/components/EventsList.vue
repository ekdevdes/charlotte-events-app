  ...
  <div class="event-view">
    <div class="row">
      <div class="col-xs-12 event-view__header-wrapper">
        ...
        <h3 class="event-view__subhead">
          {{ currentMonthName }} {{ suffix(dates.current.getDate()) }} -
          {{ nextWeekMonthName }} {{ suffix(dates.nextWeek.getDate()) }}
        </h3>
      </div>
    </div>

    <div class="row collapse event-view__event-list">
      <md-progress-spinner md-mode="indeterminate" :md-stroke="3" :md-diameter="40" v-if="loading"></md-progress-spinner>
    ...
    <div class="row">
      <div class="col-xs-12 event-view__header-wrapper">
        ...
        <h3 class="event-view__subhead">
          {{ nextWeekMonthName }} {{ suffix(dates.nextWeek.getDate()) }} -
          {{ twoWeekMonthName }} {{ suffix(dates.twoWeeks.getDate()) }}
        </h3>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      nextWeekMonthName() {
        return this.dates.nextWeek.toLocaleString('en-us', { month: 'long' })
      },
      twoWeekMonthName() {
